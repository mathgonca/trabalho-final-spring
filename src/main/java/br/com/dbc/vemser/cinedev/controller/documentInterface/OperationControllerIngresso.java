...
public interface OperationControllerIngresso {
    ...
    @Operation(summary = "Listagem de Ingressos comprados por Cliente ", description = "Lista o banco de ingressos comprados pelo Cliente, a partir da IdCliente")
    ...
    List<IngressoCompradoDTO> listarIngressosComprados(Integer id) throws RegraDeNegocioException, BancoDeDadosException;

    @Operation(summary = "Listagem de Ingressos por ID ", description = "Lista os Ingressos por IdIngresso")
    @ApiResponses(value = {@ApiResponse(responseCode = "200", description = "Retorna a lista de Ingressos comprados de acordo com a pesquisa"),
            @ApiResponse(responseCode = "403", description = "A algo de errado com as inserções de sua pesquisa"),
            @ApiResponse(responseCode = "500", description = "Foi gerada uma exceção")})
    IngressoDTO listarIngressosPorId(@PathVariable("idIngresso") Integer id) throws RegraDeNegocioException, BancoDeDadosException;

    @Operation(summary = "Listar todos os Ingressos ", description = "Lista os Ingressos referentes a busca geral do banco")
    @ApiResponses(value = {@ApiResponse(responseCode = "200", description = "Retorna a lista de Ingressos de acordo com a pesquisa"),
            @ApiResponse(responseCode = "403", description = "A algo de errado com as inserções de sua pesquisa"),
            @ApiResponse(responseCode = "500", description = "Foi gerada uma exceção")})
    List<IngressoDTO> listarIngressos() throws RegraDeNegocioException, BancoDeDadosException;
    ...
    @ApiResponses(value = {@ApiResponse(responseCode = "200", description = "Atualização de Ingresso realizada com sucesso!"),
            @ApiResponse(responseCode = "403", description = "Você não tem permissão para acessar este recurso!!"),
            @ApiResponse(responseCode = "500", description = "Foi gerada uma exceção")})
    ResponseEntity<IngressoCompradoDTO>  comprarIngresso(@PathVariable("idCliente") Integer idCliente,@PathVariable("idIngresso") Integer idIngresso) throws RegraDeNegocioException, BancoDeDadosException;
    ...
    @ApiResponses(value = {@ApiResponse(responseCode = "200", description = "Remoção do Ingresso realizada com sucesso!"),
            @ApiResponse(responseCode = "403", description = "Você não tem permissão para acessar este recurso!!"),
            @ApiResponse(responseCode = "500", description = "Foi gerada uma exceção")})
    ResponseEntity<Void> removeIngresso(@PathVariable("idIngresso") Integer id) throws RegraDeNegocioException, BancoDeDadosException;
